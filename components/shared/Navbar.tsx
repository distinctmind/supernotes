...
const Navbar = () => {
  ...
  const [navLink, setNavLink] = useState(pathname === "/notes" ? "/" : "/notes");
  ...
  return (
    <div className='pt-[1em] pl-[1em] flex items-center'>
        ...
        <div className='flex flex-col m-auto text-center gap-3'>
          <Link onClick={switchViews} href={navLink}><h2 className={cn( theme === 'day' ? 'text-black' : 'text-white', 'font-bold sm:text-[30px]')}>What&apos;s on your mind?</h2></Link> 
        </div>
    </div>
  )
}
